...
import 'package:rhythearch/common/max_value.dart';
import 'common/url.dart';
import 'common/select_list.dart';
import 'common/search_option.dart';

class AddRhythm extends StatefulWidget {
  const AddRhythm({super.key});
  @override
  // ignore: library_private_types_in_public_api
  _RhythearchAppState createState() => _RhythearchAppState();
}

class _RhythearchAppState extends State<AddRhythm> {
  List<int> rhythm = [];
  ...
  // リズム追加結果
  static var addRhythmResultText = "";
  ...
  Future<void> addRhythm() async {
    if (rhythm[rhythm.length - 1] > maxTime) {
      rhythm = [];
      setState(() {
        isRecording = false;
        isButtonVisible = false;
        addRhythmResultText = "リズムは100秒以内で入力してください";
      });
      return;
    }
    ...
    await sendRhythm(rhythm, _titleFieldController.text,
  ...
  Future<void> sendRhythm(List<int> rhythm, String title, String artist,
      String voiceType, String category, String soundType) async {
    if (title.isEmpty || artist.isEmpty) {
      setState(() {
        addRhythmResultText = "タイトルとアーティストは必須です";
      });
      return;
    }
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        isRecording = false;
        isButtonVisible = false;
        addRhythmResultText = "リズムを追加しました";
      });
      return;
    } else {
      // エラーレスポンス
      setState(() {
        addRhythmResultText = "リズムの追加に失敗しました";
      });
      return;
    }
  }
                  ...
                  showDialog(
                    ...
                    builder: (context) {
                      return AlertDialog(
                        ...
                        content: Text(addRhythmResultText),
                        actions: [
                          TextButton(
                            child: const Text("OK"),
